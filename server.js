document.addEventListener('DOMContentLoaded', () => {
    ...
    function initializePeerJS() {
        try {
            ...
            peer.on('error', (err) => showError(`A P2P error occurred: ${err.type}`));
        } catch (e) {
            showError("Failed to initialize P2P connection.");
        }
    }
    ...
    function setupReceiverConnection(conn) {
        ...
        dataConnection.on('close', () => {
            if (receivedSize < totalFileSize && totalFileSize > 0) {
                showError("Connection lost. Transfer failed.");
    ...
    socket.on('peer-disconnected', (data) => {
        if (!isSender) {
            showError(data.message || "The other user has disconnected.");
            clearInterval(speedInterval);
    ...
    uploadArea.addEventListener('dragover', (e) => { e.preventDefault(); uploadArea.classList.add('hover:bg-zinc-100'); });
    uploadArea.addEventListener('dragleave', () => uploadArea.classList.remove('hover:bg-zinc-100'));
    uploadArea.addEventListener('drop', (e) => { e.preventDefault(); uploadArea.classList.remove('hover:bg-zinc-100'); if (e.dataTransfer.files.length) handleFile(e.dataTransfer.files[0]); });
